...
public static class TutorialContent
{
    ...
    [
        ...
        new(
            "7. 安全功能和更新",
            "怎么做：需要定时停止时，先开始分享，再点击“定时停止”，输入分钟数并开始计时。看到更新提示时，点击更新后 FlowCast 会关闭旧版本、安装新版本并自动重新打开。\n为什么：定时停止可避免忘记结束分享；更新会先验证文件，再用 Setup 替换旧程序。\n成功后：定时到点会停止分享并取消勾选；更新完成后自动打开的就是新版本。\n出问题时：如果状态显示“需要处理”，不要继续分享，点击“停止分享”或重新打开 FlowCast，让程序先回到只自己听。",
            ["定时停止", "自动更新", "自动重新打开"],
            ["只有主动点击“定时停止”后，才会开始计时。", "更新下载完成后，FlowCast 会自动安装并重新打开。"]),
    ];
}
