...
public static class TutorialContent
{
    ...
    [
        ...
        new(
            "5. 选择要分享的程序",
            "怎么做：先让要分享的程序开始播放声音，再回到 FlowCast 点“刷新”。勾选要分享的程序，点击“开始分享”，等待倒数结束。\n为什么：FlowCast 会把勾选的程序送到 Input；没有勾选的程序保持只自己听。\n成功后：朋友只会听到你勾选的程序，例如 Chrome 或网易云音乐。\n出问题时：程序没有出现时，让它先播放声音，再点刷新。",
            ["勾选程序", "开始分享", "朋友听到"],
            ["没有勾选任何程序时，开始分享不能按。"]),
        new(
            "6. 静音、最小化和停止分享",
            "怎么做：分享中可点“静音分享”临时不让朋友听音乐；再点“恢复分享”即可继续。点窗口右上角 X 会最小化到系统托盘，双击托盘图标可重新打开。要结束分享时点“停止分享”。\n为什么：静音不会影响你自己的播放；系统托盘让分享继续而不占桌面。\n成功后：停止分享后，你仍然听得到声音，Discord 朋友听不到这些程序。\n出问题时：如果朋友还听得到，重新打开 FlowCast，点停止分享后再确认一次。",
            ["静音分享", "系统托盘", "停止分享"],
            ["X 只是最小化；只有停止分享或结束 FlowCast 才会结束分享。"]),
        new(
            "7. 定时、设置和更新",
            "怎么做：需要定时停止时，先开始分享，再点“定时”选择分钟数。设置里可改开始倒数、是否在停止时恢复本机播放、以及结束提示音。看到更新提示时，点更新后 FlowCast 会关闭旧版本、安装新版本并自动重新打开。\n为什么：定时停止可避免忘记结束分享；设置让你决定需要的提醒方式。\n成功后：定时到点会停止分享并取消勾选；更新完成后自动打开的就是新版本。\n出问题时：如果状态显示“需要处理”，不要继续分享，点停止分享或从系统托盘选择结束 FlowCast，让程序先回到只自己听。",
            ["定时停止", "设置", "自动更新"],
